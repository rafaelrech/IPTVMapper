...
public class M3U8PlaylistParser extends AbstractParser {
	 ...
	 */
	private void parsePlaylist(String uri, InputStream stream, Playlist playlist)
		...
		while ((line = reader.readLine()) != null) {
			if (!(line.equalsIgnoreCase(EXTENDED_INFO_TAG) || line.matches(INFO_TAG) || line.trim().equals(""))) {
	...
	private String getHost(String uri) throws JPlaylistParserException {
		try {
			...
			if (index > -1) {
				host = new URI(host.getScheme(), null, host.getHost(), host.getPort(), path.substring(0, index + 1),
						null, null);
			}
